    ...
    <div className="cart-item">
      {onAddFavorite && (
        <img
          ...
          src={isAddFavorite(id) ? "img/forever-add.svg" : "img/forever.svg"}
          alt="forever"
          className="cart-f cu-p"
        />
      )}
      <a href={"product/" + id}>
        {" "}
      ...
      <div className="d-flex justify-between align-center">
        <div className="cart-block">
          <p>Цена:</p>
          <b className="cart-price">{price} тг.</b>
        </div>
        {onAddItem && (
          <img
            width={32}
            height={32}
            onClick={onAddItems}
            src={isAddItem(id) ? "img/add-check.svg" : "img/btn-plus.svg"}
            alt="btn-plus"
            className="btn-plus cu-p"
          />
        )}
      </div>
    </div>
